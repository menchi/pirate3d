...
SMesh* MDLFileLoader::CreateMesh(FileReader* file)
{
	...
	pMeshBuffer->GetVertexBuffer(0)->Lock(0, 0, (void**)&pVert, 0);
	for (int i=0; i<pStripGroup->numVerts; i++)
	{
		Vector* p = pVertexdata->Position(pStripGroup->pVertex(i)->origMeshVertID);
		Vector* n = pVertexdata->Normal(pStripGroup->pVertex(i)->origMeshVertID);
		Vector2D* t = pVertexdata->Texcoord(pStripGroup->pVertex(i)->origMeshVertID);
		pVert[i].Position[0] = p->x; pVert[i].Position[1] = p->y; pVert[i].Position[2] = p->z;
		pVert[i].Normal[0] = n->x; pVert[i].Normal[1] = n->y; pVert[i].Normal[2] = n->z;
		pVert[i].Texcoord[0] = t->x; pVert[i].Texcoord[1] = t->y;
	}
	pMeshBuffer->GetVertexBuffer(0)->Unlock();
	...
	memcpy(pIndices, pStripGroup->pIndex(0), pStripGroup->numIndices*sizeof(u16));
	pMeshBuffer->GetIndexBuffer()->Unlock();

	mstudiotexture_t* pTexture =  pMDLFile->pTexture(0);
