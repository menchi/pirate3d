#ifndef _PIRATE_STRING_H_
#define _PIRATE_STRING_H_

#include "pirateTypes.h"
#include "pirateAllocator.h"
...
*/
template <typename T, typename TAlloc = AllocatorDefault<T> >
class string
{
public:
	...
	string(double number) : array(0), allocated(0), used(0)
	{
		c8 tmpbuf[255];
	...
	{
		_PIRATE_DEBUG_BREAK_IF(index>=used) // bad index
	...
	{
		start = clamp ( start < 0 ? (s32)(used) - 1 : start, 0, (s32)(used) - 1 );
	...
	{
		_PIRATE_DEBUG_BREAK_IF(index>=used) // access violation
